...
// Atiende todas las peticiones del cliente en un bucle hasta que cierre la conexión
static void atender_cliente(int descriptor_cliente) {
    // Bucle: seguimos atendiendo peticiones del mismo cliente hasta que cierre
    while (1) {
        ...
        // Si falla, el cliente cerró la conexión: salimos del bucle
        ...
        if (longitud_cuerpo > 0) {
            ...
            // Si falla malloc, respondemos con error y salimos
            if (cuerpo == NULL) {
                (void)enviar_respuesta(descriptor_cliente, operacion, -1, NULL, 0);
                break;
            }
    ...
    } // fin del bucle de peticiones

    // Cerramos la conexión con este cliente cuando termina el bucle
